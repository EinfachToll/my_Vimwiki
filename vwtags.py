...
if len(sys.argv) > 2:
	...
	import os
	filenames = [ f for f in os.listdir('.') if f.endswith(fileextension) ]
...
for filename in filenames:
	...
	for lnum, line in enumerate(file_content):
		...
		tag_name = filename.replace(fileextension, '') + '#' + cur_tag
		outputlist.append('{0}\t{1}\t/{2}/;"\t{3}\tline:{4}{5}'.format(tag_name, filename, cur_searchterm, cur_kind, str(lnum+1), scope))

outputlist.sort()
with open('tags', 'w') as datei:
	datei.write("\n".join(outputlist))
